...
class Customer(models.Model):
    ...
    qq = models.CharField(max_length=64, unique=True, help_text=u'QQ号必须唯一')  # QQ号64字节，且设置成唯一---CharField【字符类型】
    qq_name = models.CharField(u'QQ名称', max_length=64, blank=True, null=True)
    # 客户只要没报名，没理由要求人家必须告诉你真实姓名及其它更多私人信息
    name = models.CharField(u'姓名', max_length=32, blank=True, null=True)  # 姓名32字节，汉字最多存储10个汉字---CharField
    sex_type = (('male', u'男'), ('female', u'女'))
    sex = models.CharField(u"性别", choices=sex_type, default='male', max_length=32)  # 性别32字节，设置可选择值并设置默认值---CharField
    birthday = models.DateField(u'出生日期', max_length=64, blank=True, null=True, help_text="格式yyyy-mm-dd")  # 出生日期64字节--DateField【日期格式】
    phone = models.BigIntegerField(u'手机号', blank=True, null=True)  # 手机号---BigIntegerField【长整型(有符号的)】
    ...
    # 新客户都是老学员转介绍来了，如果是转介绍的，就在这里纪录是谁介绍的他，前提这个介绍人必须是老学员噢，要不然系统里找不到
...
class CustomerFollowUp(models.Model):
    ...
    status = models.IntegerField(u"跟进状态", choices=status_choices, help_text=u"选择客户此时的状态")
